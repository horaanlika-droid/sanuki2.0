...
def register_handlers(dp):
    ...
    async def start(message: Message, state: FSMContext):
        ...
        await state.set_state(Registration.waiting_name.state)

    @dp.message_handler(state=Registration.waiting_name)
    ...
    # ========== НАЧАЛО ЗАКАЗА ==========
    @dp.callback_query_handler(text=["dine_in", "takeaway", "delivery"])
    async def start_order(callback: CallbackQuery, state: FSMContext):
        ...
        await state.set_state(OrderStates.choosing_order_type.state)
    ...
    # ========== ВЫБОР ОСНОВЫ ==========
    @dp.callback_query_handler(text_startswith="base_", state=OrderStates.choosing_order_type)
    ...
    # ========== ВЫБОР БЕЛКА ==========
    @dp.callback_query_handler(text_startswith="protein_", state=OrderStates.choosing_base)
    ...
    # ========== ВЫБОР ТОППИНГА ==========
    @dp.callback_query_handler(text_startswith="topping_", state=OrderStates.choosing_protein)
    ...
    @dp.callback_query_handler(text_startswith="extra_", state=OrderStates.choosing_topping)
    ...
    # ========== КОРЗИНА ==========
    @dp.callback_query_handler(text="go_to_cart", state=OrderStates.choosing_topping)
    async def show_cart(callback: CallbackQuery, state: FSMContext):
        ...
        await state.set_state(OrderStates.viewing_cart.state)
    ...
    @dp.callback_query_handler(text="add_more", state=OrderStates.viewing_cart)
    ...
    @dp.callback_query_handler(text="clear_cart", state=OrderStates.viewing_cart)
    ...
    @dp.callback_query_handler(text="checkout", state=OrderStates.viewing_cart)
    async def checkout(callback: CallbackQuery, state: FSMContext):
        data = await state.get_data()
        total = data.get("total", 0)
        
        await state.set_state(OrderStates.confirming.state)
        await callback.message.edit_text(
    ...
    @dp.callback_query_handler(text="back_to_cart", state=OrderStates.confirming)
    async def back_to_cart(callback: CallbackQuery, state: FSMContext):
        await state.set_state(OrderStates.viewing_cart.state)
        await show_cart(callback, state)

    @dp.callback_query_handler(text="confirm_order", state=OrderStates.confirming)
    ...
    # ========== ОБРАБОТЧИКИ АДМИНА ==========
    @dp.callback_query_handler(text_startswith="admin_accept_")
    async def admin_accept(callback: CallbackQuery):
        order_id = int(callback.data.split("_")[2])
        await update_order_status(order_id, "принят", callback)
        await callback.answer("Заказ принят")

    @dp.callback_query_handler(text_startswith="admin_cook_")
    ...
    @dp.callback_query_handler(text_startswith="admin_ready_")
    ...
    @dp.callback_query_handler(text_startswith="admin_cancel_")
    ...
    @dp.callback_query_handler(text="back_to_order_type", state=OrderStates.choosing_order_type)
    ...
    @dp.callback_query_handler(text="back_to_bases", state=OrderStates.choosing_base)
    ...
    @dp.callback_query_handler(text="back_to_proteins", state=OrderStates.choosing_protein)
    async def back_to_proteins(callback: CallbackQuery, state: FSMContext):
        ...
        await state.set_state(OrderStates.choosing_base.state)
        await callback.answer("Назад")
        await callback.message.edit_text(
            f"<b>Основа:</b> {base_name}\n\n"
    ...
    @dp.callback_query_handler(text="back_to_toppings", state=OrderStates.choosing_topping)
    async def back_to_toppings(callback: CallbackQuery, state: FSMContext):
        await state.set_state(OrderStates.choosing_protein.state)
        await callback.answer("Назад")
        menu = await load_menu()
        toppings = [item for item in menu["categories"] if item["name"] == "UDON"][0]["toppings"]
        await callback.message.edit_text(
